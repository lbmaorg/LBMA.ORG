 ...
 * Blog Template
 *
   Template Name: Press Release (full posts)
 *
...
<?php
	...
	elseif ( get_query_var('page') ) 
		...
		$paged = 1;
		query_posts("post_type=post&paged=$paged&cat=10"); 
?>    

<h1 class="pressroom_title">PRESS ROOM</h1>
...
<p class="timestamp">Posted on <?php the_time('F jS, Y') ?></p>
                    <div class="post-content"><?php the_content(__('Read more &#8250;', 'responsive')); ?></div>
                    <?php wp_link_pages(array('before' => '<div class="pagination">' . __('Pages:', 'responsive'), 'after' => '</div>')); ?>
                </div><!-- end of .post-entry -->
                
                <div class="post-data">
				   <?php /*?> <?php the_tags(__('Tagged with:', 'responsive') . ' ', ', ', '<br />'); ?> 
					<?php printf(__('Posted in %s', 'responsive'), get_the_category_list(', ')); ?> <?php */?>
